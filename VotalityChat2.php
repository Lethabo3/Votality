...
function handleSendMessage($message, $chatId, $file = null, $timezone = 'UTC') {
    ...
    try {
        ...
        // Always return response in the correct format
        return [
            'response' => $aiResponse,
            'chatId' => $chatId,
            'relatedTopics' => $relatedTopics,
...
class OfflineVotalityAI {
  ...
  generateResponse(message, chatId) {
    ...
    const response = `This is an offline response to: "${message}". In a full implementation, we would use a pre-trained model or a more sophisticated algorithm to generate responses.`;
    ...
    return {
      response: response,
      chatId: chatId,
      relatedTopics: ['Offline AI', 'Browser-based chatbots', 'Local storage'],
    };
  }
  ...
  createNewChat() {
    ...
    return { chatId, summary: 'New Offline Chat' };
  }

  getRecentChats() {
    // In a real implementation, this would retrieve chats from local storage
    return {
      chats: [
        { chat_id: 'chat_1', summary: 'Offline Chat 1', created_at: new Date().toISOString() },
        { chat_id: 'chat_2', summary: 'Offline Chat 2', created_at: new Date().toISOString() },
      ]
    };
  }

  // Simulated market data (in a real implementation, this could be periodically updated when online)
  getMarketData() {
    return {
      marketData: {
        'AAPL': { price: 150.25, change: 2.5 },
        'GOOGL': { price: 2750.10, change: -5.2 },
        'MSFT': { price: 305.75, change: 1.8 },
      }
    };
  }
}
...
// Usage example
const offlineAI = new OfflineVotalityAI();

console.log("Creating a new chat:");
const newChat = offlineAI.createNewChat();
console.log(newChat);

console.log("\nGenerating a response:");
const response = offlineAI.generateResponse("Hello, what's the latest market news?", newChat.chatId);
console.log(response);

console.log("\nGetting recent chats:");
const recentChats = offlineAI.getRecentChats();
console.log(recentChats);

console.log("\nGetting market data:");
const marketData = offlineAI.getMarketData();
